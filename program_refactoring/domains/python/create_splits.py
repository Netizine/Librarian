...
for dataset_path in Path("third_party/Faithful-COT/data/").glob("*"):
    ...
    for model_path in Path(output_path).glob("*"):
        ...
        outputs = [
            json.loads(x) for x in open(model_path / "predictions.jsonl").readlines()
        ]
        ...
        train_data = [(data_to_keep[i], outputs_to_keep[i]) for i in train_idxs]
        dev_data = [(data_to_keep[i], outputs_to_keep[i]) for i in dev_idxs]
        test_small_data = [
            (data_to_keep[i], outputs_to_keep[i]) for i in test_small_idxs
        ]
        test_data = [(data_to_keep[i], outputs_to_keep[i]) for i in test_idxs]

        # get data also with incorrect for dev and test
        correct_only_orig_idxs = [
            x[0] for x in train_data + dev_data + test_small_data + test_data
        ]
        incorrect_also_idxs = [
            i for i in range(len(data)) if i not in correct_only_idxs
        ]
        ...
        dev_incorrect_idxs = incorrect_also_idxs[
            0 : int(0.5 * len(incorrect_also_idxs))
        ]
        test_incorrect_idxs = incorrect_also_idxs[int(0.5 * len(incorrect_also_idxs)) :]

        # add the two splits together
        dev_with_incorrect_data = [
            ((i, data[i]), (i, outputs[i]))
            for i in dev_incorrect_idxs + dev_correct_only_idxs
        ]
        test_with_incorrect_data = [
            ((i, data[i]), (i, outputs[i]))
            for i in test_incorrect_idxs + test_correct_only_idxs
        ]

        Path(outpath / dataset_name / model_name).mkdir(parents=True, exist_ok=True)

        with (
            open(outpath / dataset_name / model_name / "train_data.jsonl", "w") as f1,
            open(
                outpath / dataset_name / model_name / "train_outputs.jsonl", "w"
            ) as f2,
            open(
                outpath / dataset_name / model_name / "train_combined.jsonl", "w"
            ) as f3,
        ):
        ...
        with (
            open(outpath / dataset_name / model_name / "dev_data.jsonl", "w") as f1,
        ...
        with (
            open(
                outpath / dataset_name / model_name / "test_small_data.jsonl", "w"
            ) as f1,
            open(
                outpath / dataset_name / model_name / "test_small_outputs.jsonl", "w"
            ) as f2,
            open(
                outpath / dataset_name / model_name / "test_small_combined.jsonl", "w"
            ) as f3,
        ):
        ...
        with (
            open(outpath / dataset_name / model_name / "test_data.jsonl", "w") as f1,
        ...
        with open(
            outpath / dataset_name / model_name / "dev_w_incorrect_data.jsonl", "w"
        ) as f1:
        ...
        with open(
            outpath / dataset_name / model_name / "test_w_incorrect_data.jsonl", "w"
        ) as f2:
