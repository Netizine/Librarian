    ...
    def conversion_helper(chunk, d=0, use_self=False):
        ...
        if isinstance(chunk[0], Symbol):
            if chunk[0]._val == "loop":
                ...
                return (
                    f"{conversion_helper(angle1, d)} - {conversion_helper(angle2, d)}"
                )
        ...
        # recursion case: chunk is a list, convert each element
            ...
            return "\n".join(
                [conversion_helper(c, d, use_self=use_self) for c in chunk]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="path to human language. If provided, human language will be used!",
    )
