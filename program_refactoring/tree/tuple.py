...
class Tuple:
    def __init__(self, nodes: Dict[int, Node], task: str = "logos"):
        self.nodes = nodes
        self.task = task
        self.original_nodes = {
            k: v.__class__.from_json(v.to_json()) for k, v in nodes.items()
        }
    ...
    def run_self_consistency(self, model, prompt, n_variants=3):
        ...
        for i in range(n_variants):
            for idx in programs_variants_by_idx.keys():
                ...
                program_str = "\n".join(program_str)
                prompt = python_self_consistency_prompt.format(
                    query=self.nodes[idx].query,
