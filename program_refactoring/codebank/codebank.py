...
from program_refactoring.domains.logos.utils import (
    get_func_names as get_logo_func_names,
)
from program_refactoring.domains.logos.utils import (
    make_pass_fail_str as make_logo_pass_fail_str,
)
from program_refactoring.domains.python.utils import (
    get_func_names as get_python_func_names,
)
from program_refactoring.domains.textcraft.utils import (
    get_func_names as get_textcraft_func_names,
)
...
SINGLE_PROMPTS = {
    "logos": logo_codebank_single_refactor_prompt,
...
class CodeBank:
    def __init__(
        ...
        if task == "python" or task == "codecontests":
            self._imports = [
        ...
        if not skip_collection:
            self.openai_ef = embedding_functions.OpenAIEmbeddingFunction(
                ...
                # TODO: switch to text-embeeding-3-small?
                model_name="text-embedding-ada-002",
                # model_name="text-embedding-3-small",
            )
            if run_dir is None and len(logger.manager.root.handlers) > 0:
                logdir = Path(
                    logger.manager.root.handlers[0].baseFilename.split(".log")[0]
                )
            else:
            ...
            self.collection = self.chroma_client.get_or_create_collection(
                name=name, embedding_function=self.openai_ef
            )
    ...
    def reset(self):
        self.chroma_client.delete_collection(self._name)
        self.collection = self.chroma_client.get_or_create_collection(
            name=self._name, embedding_function=self.openai_ef
        )
    ...
    def add_multiple(self, big_str, round_added=None):
        ...
        for node in ast.walk(parsed):
            if isinstance(node, ast.FunctionDef):
                ...
                # might run into syntax errors. if that happens, just give up
                try:
                    self.add(func_code, round_added)
    ...
    def save(self, path):
        ...
        with open(path.parent / "success_info.json", "w") as f1:
            ...
            for func in self._codebank.values():
                line = {
                    "name": func._name,
                    "was_success": func.was_success,
                    "num_programs_used": func.num_programs_used,
    ...
    def load(
        ...
        codebank.add_multiple(data)  # this is extremely slow

        if succpath is not None:
            # read success info
        ...
        # read test cases
        if testcase_path is not None and Path(testcase_path).exists():
        ...
        # if succ_data is empty, then this will shortcircuit
        ...
        for line, tc_line in zip(succ_data, testcase_data):
            try:
                ...
                codebank._codebank[line["name"]].num_programs_used = line[
                    "num_programs_used"
                ]
                # add test cases
                test_cases = []
    ...
    def clone(cls, codebank):
        ...
        new_codebank._imports = codebank._imports
        for k, v in new_codebank._codebank.items():
            assert v.was_success == codebank._codebank[k].was_success

        new_codebank._by_description = {
            k: v for k, v in codebank._by_description.items()
        }
    ...
    def write_to_file(self, filename=None):
        if filename is None:
            filename = f"{self.temp_dir}/codebank.py"
    ...
    def str(self):
        ...
        code = "\n\n".join([f._original_code for f in self._codebank.values()])
    ...
    def filter(
        ...
        for func in self._codebank.values():
            ...
            if success > success_thresh and n_used >= min_usage:
                logger.info(
                    f"\t{func._name} has success score of {success} and was used {n_used} times, keeping"
                )
                funcs_to_keep.append(func._name)
            elif n_used < min_usage and n_used > 0 and round_delta > max_round_delta:
                logger.info(
                    f"\t{func._name} has success score of {success} and was used {n_used} times. It has been {round_delta} since it was introduced, removing"
                )
            elif n_used < min_usage and n_used > 0 and keep_low_usage:
                logger.info(
                    f"\t{func._name} has success score of {success} and was used {n_used} times, but was used too little to know if good, keeping"
                )
                funcs_to_keep.append(func._name)
            else:
                logger.info(
                    f"\t{func._name} has success score of {success} and was used {n_used} times, removing"
                )

        # remove anything that doesn't succeed more than half the time (thresh = 0.0)
        removed = [
            func for func in self._codebank.values() if func._name not in funcs_to_keep
        ]
...
Before refactoring, please describe the operations you will perform in the following format, followed by the new program:"""
        ...
        def extract_output(output):
            ...
            match = re.search(r"NEW PROGRAM:?\s*```python(.*?)```", output, re.DOTALL)
    ...
    def refactor_one_function(
        ...
        passing_cases = [
            ...
            if self._codebank[func_name].was_success[i]
        ]
        failing_cases = [
            tc
            for i, tc in enumerate(test_cases)
            if not self._codebank[func_name].was_success[i]
        ]
        ...
        for pc in passing_cases:
            ...
            if (
                pc.pred_node.query not in all_queries
                and pc.pred_node.program not in all_progs
            ):
        ...
        for fc in failing_cases:
            ...
            if (
                fc.pred_node.query not in all_queries
                and fc.pred_node.program not in all_progs
            ):
        ...
        if len(failing_cases) == 0:
            logger.info(
                f"Function {func._name} is passing all tests, no refactor needed for now"
            )
            called = [
                x
                for tc in passing_cases
                for x in self.get_func_names(tc.pred_node.program, self.str())
            ] + [func_name]
            return None, called

        logger.info(
            f"Before refactor, {func._name} is passing {old_passing_perc * 100:.1f}% of cases"
        )
        ...
        for iteration in range(n_iter):
            ...
            # refactor program
            new_func_mappings = self.single_func_refactor(
                func_to_refactor,
                passing_cases,
                failing_cases,
                header,
            ...
            if len(new_func_mappings) == 0:
                # misparsed!
                logger.info("Could not parse any outputs")
                break
            ...
            for new_func_mapping in new_func_mappings:
                ...
                for i, test_case in enumerate(test_cases):
                    ...
                    passing_before = test_case_copy_before.get_acc(
                        task=task, overwrites=[func_to_refactor]
                    )
                    logger.info(f"Passing: {passing_before}")
                    if iteration == 0:
                        # get functions originally called in all test cases
                        old_called += self.get_func_names(
                            test_case_copy_before.pred_node.program,
                            self.str(),
                        )
                    ...
                    for new_fn in new_func_mapping.keys():
                        try:
                            ...
                            try:
                                signature_differs = check_args(
                                    self._codebank[new_fn], new_func_mapping[new_fn]
                                )
                    ...
                    passing_after = test_case_copy_after.get_acc(
                        task=task, overwrites=list(new_func_mapping.values())
                    )
                ...
                if len(passing_cases) >= len(new_passing_cases):
                    ...
                    logger.info(
                        f"Old function had more/same passing cases: {len(passing_cases)} > {len(new_passing_cases)}"
                    )
                    new_func = func_to_refactor
                    continue
                else:
                    # otherwise, update and repeat
                    logger.info("New version is better, updating!")
                    try:
        ...
        if updated:
            ...
            for i, test_case_copy in enumerate(test_cases):
                ...
                called += self.get_func_names(test_case_copy.pred_node.program, self.str())
    ...
    def refactor(
        ...
        # JUSTIN: i think this is synthesizing new functions? wizardcoder style
        ...
        # TODO: parallelize and make async
        for func_name, func in mapping.items():
            result, called = self.refactor_one_function(
                func_name, func, header=header, task=task, round_added=round_added
            )
        ...
        for func in self._codebank.values():
            ...
            funcs_called_in_codebank.extend(self.get_func_names(code, self.str()))
        ...
        for fn in codebank_keys:
            if (
                fn not in to_update
                and fn not in funcs_called_in_passing_tests
                and fn not in funcs_called_in_codebank
            ):
    ...
    def add_comments(self):
        ...
        def remove_imports(code_block):
            code_block = re.sub(
                r"from .* import .*", "", code_block, flags=re.MULTILINE
            )
        ...
        for func in self._codebank.keys():
            ...
            passing_cases = [
                tc
                for i, tc in enumerate(test_cases)
                if self._codebank[func].was_success[i]
            ]
            comment_prompt = self.comment_prompt.format(
                function=self._codebank[func]._original_code
            )
