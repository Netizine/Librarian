...
class TestCase:
    ...
    def parse_response(self, output):
        ...
        output = "\n".join(
            re.findall(r"```python(.*)?```", output, flags=re.DOTALL | re.MULTILINE)
        )
    ...
    def refactor(self, old_to_new_mapping):
        ...
        logger.info("Signature changed, refactoring test case")
        change_str = []
        ...
        prompt = test_case_refactor_prompt.format(
            func_change_str=change_str, program=program_str, query=self.pred_node.query
        )
        ...
        try:
            self.pred_node.exec_program = self.pred_node.wrap_program(
                self.pred_node.program
            )
...
class CodeContestTestCase(TestCase):
    def get_acc(self, task: str, overwrites: List[Function]) -> bool:
        try:
            parsed = ast.parse(self.pred_node.exec_program)
        except Exception:
            return False
        ...
        try:
            pred = self.pred_node.execute()
            pred_passed = sum(pred.passed)
        except:
            pred_passed = 0
        try:
            gold = self.gold_node.execute()
            gold_passed = sum(gold.passed)
        except:
            gold_passed = 0

        return pred_passed >= gold_passed
    ...
    def from_json(cls, data):
        ...
        data["pred_node"]["type"] = "pred"
        data["gold_node"]["type"] = "gold"
        pred_node = CodeContestNode.from_json(data["pred_node"])
        model = OpenAIModel.from_json(data["model"])
        gold_node = CodeContestNode.from_json(data["gold_node"])
        # expected = np.array(data['expected'])
        return cls(pred_node, gold_node, model)
