...
class Function:
    ...
    def from_str(cls, function_str, round_added=None):
        ...
        try:
            ...
            raise SyntaxError(
                f"Could not parse function from {function_str} because of SyntaxError"
            )

        # don't add non-functions to the codebank
        if len(parsed.body) > 1:
            raise ValueError("Function class cannot parse multiple functions at once")
        ...
        if len(split_str) == 1:
            # shortcut out, since the rest of the program assumes multi-line
            return cls(name, args, content, name, function_str, round_added)
    ...
    def summarize(self, include_ex=False, include_success=True):
        ...
        if include_ex:
            ...
            successful_tcs = [
                x for i, x in enumerate(self.test_cases) if self.was_success[i]
            ]
            if len(successful_tcs) > 0:
                successful_tcs = sorted(
                    successful_tcs, key=lambda x: len(x.pred_node.program.split("\n"))
                )
                ...
                ex_prog = "\n".join(
                    [f"    >>> {x.strip()}" for x in ex_prog.split("\n")]
                )
