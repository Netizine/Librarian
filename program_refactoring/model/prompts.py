python_tuple_refactor_prompt = """Please rewrite the following programs by extracting helper functions.
...
old_python_tuple_refactor_prompt = """Please rewrite the following two programs to be more efficient. 
...
Return the code in Markdown: ```python ... ```.
{func_change_str}
Query: {query}
Old test case: 
{program}
...
REFACTOR_TEMPLATE = '''# Python Code Refactoring Guidelines

Please rewrite the following programs by extracting helper functions to improve code organization and reusability.

## Requirements:
- Create helper functions to reduce code size and improve readability
- DO NOT use classes
- DO NOT use nonlocal variables (rewrite algorithms if necessary)
- DO NOT create nested helper functions (write them as separate functions)
- Main functions should remain as they are (don't extract them)
- Make helper functions as general and reusable as possible
- Use descriptive names for all functions
...
Here are helper functions you should use:
{helpers}

## Output Format

Your answer must follow this exact Markdown structure:

### 1. Codebank
```python
# Place all helper functions here
def helper_function1(param1, param2):
    """Brief description of function purpose"""
    # Implementation
    return result

def helper_function2(param1, param2, param3):
    """Brief description of function purpose"""
    # Implementation
    return result
```
...
### 2. Main Programs
For each query, include the refactored main program using your helper functions:

#### Program 1
```python
from {temp_dir}.codebank import *

# Constants declared here (not in codebank)
CONSTANT = 1

def main():
    # Implementation using helper functions
    result = helper_function1(arg1, arg2)
...
## Important Reminders
- Helper functions should be GENERIC and REUSABLE across programs when possible
- Constants stay in main programs, not in the codebank
- Each helper function should have a SINGLE RESPONSIBILITY
- ELIMINATE ALL nonlocal variables - redesign logic if needed
- Use DESCRIPTIVE NAMES that indicate function purpose
- Include brief docstrings for all helper functions
```'''

RETRIEVE_CODEBANK_TEMPLATE = '''# Function Selection Assistant

## Your Task
You're being presented with a programming problem, a solution plan, and a list of available helper functions. Your job is to carefully select the specific helper functions that would be most useful for implementing the given plan.

## Instructions
1. Analyze the problem statement thoroughly to understand the requirements.
2. Review the solution plan to identify the specific operations needed.
3. Examine each helper function to understand its purpose and functionality.
4. Select ONLY the helper functions that directly support the plan implementation. Include any necessary imports.
5. Do not create nested helper functions. Instead, write those as separate helper functions which can call other helper functions.
6. Format your selections as a Markdown code block with proper Python syntax.
7. Include brief explanations for why you selected each function.
...
## Output Format
Present your selected helper functions with brief explanations in a clean, properly indented Python code block like this:

```python
# Selected Helper Functions

# Selected because: The plan specifically mentions sorting the input list first
def sort_list(arr):
    """Sort a list in ascending order"""
    return sorted(arr)

# Selected because: The plan uses a two-pointer approach to find pairs that sum to target
def two_pointers_sum(arr, target):
    """Find pairs that sum to target using two pointers"""
...
## Note
Choose functions economically - only include those that are necessary and will meaningfully contribute to the solution. Include any necessary imports.
Copy the helper functions exactly. Do not change them.
'''
