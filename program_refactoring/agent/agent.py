...
from program_refactoring.model.hf_model import (
    CodeLlamaModel,
    LemurModel,
    QwenModel,
)
from program_refactoring.model.lemur_prompts import (
    lemur_logo_agent_completion_prompt,
    lemur_python_agent_completion_prompt,
    lemur_retrial_prompt,
    lemur_textcraft_agent_completion_prompt,
)
from program_refactoring.model.llama_prompts import (
...
from program_refactoring.model.prompts import (
    gpt_feedback_prompt,
    gpt_logo_agent_prompt,
    gpt_python_agent_prompt,
    gpt_retrial_prompt,
    gpt_textcraft_agent_prompt,
)
from program_refactoring.model.qwen_prompts import qwen_logo_agent_completion_prompt
...
MODEL_DICT = {
    ...
    "Qwen/Qwen2.5-14B-Instruct-1M": QwenModel,
}


COMPLETION_PROMPTS = {
    "llama": {
        "logos": llama_logo_agent_completion_prompt,
        "python": llama_python_agent_completion_prompt,
        "textcraft": llama_textcraft_agent_completion_prompt,
    },
    "lemur": {
        "logos": lemur_logo_agent_completion_prompt,
        "python": lemur_python_agent_completion_prompt,
        "textcraft": lemur_textcraft_agent_completion_prompt,
    },
    "gpt": {
        "logos": gpt_logo_agent_prompt,
        "python": gpt_python_agent_prompt,
        "textcraft": gpt_textcraft_agent_prompt,
    },
    "qwen": {"logos": qwen_logo_agent_completion_prompt},
}

RETRIAL_PROMPTS = {
    "gpt": {"textcraft": gpt_retrial_prompt},
    "llama": {"textcraft": llama_retrial_prompt},
    "lemur": {"textcraft": lemur_retrial_prompt},
}
FEEDBACK_PROMPTS = {
    "gpt": {"textcraft": gpt_feedback_prompt},
    "llama": {"textcraft": ""},
    "lemur": {"textcraft": ""},
}

NODE_DICT = {"logos": LogoNode, "python": CodeContestNode, "textcraft": TextCraftNode}
...
class Agent:
    def __init__(
        ...
        self.train_data_by_id = {
            k: {ex.id: ex for ex in train_data} for k, train_data in train_datas.items()
        }
        ...
        if self.model_type in ["llama", "lemur"]:
            ...
            self.prompt_builder = self.qwen_prompt_builder
        else:
        ...
        # TODO: maybe want to reset this? check if persists between test runs

        self.collections = {k: None for k in self.train_datas.keys()}
        for k, train_data in self.train_datas.items():
            collection = chroma_client.get_or_create_collection(
                name=f"{self.task}_agent_{k}", embedding_function=openai_ef
            )
            ...
            docs, ids = self.get_docs_ids(
                train_data, codebank, do_filter=self.task == "logos"
            )
    ...
    def retrieve_similar(self, data_key, example, k, attempts=0, exclude_current=True):
        ...
        try:
            results = self.collections[data_key].query(
                query_texts=[example.query], n_results=k
            )
        ...
        to_ret = [
            self.train_data_by_id[data_key][id]
            for id in results
            # hack because training data from test nodes seems incomplete
            if id in self.train_data_by_id[data_key]
        ]
        if exclude_current:
    ...
    def llama_prompt_builder(self, example, icl_examples, craft_retrieve=False):
        ...
        if self.codebank is not None:
            ...
            codebank_str = [
                func.summarize(
                    include_ex=self.use_docstring_exs, include_success=self.use_success
                )
                for func in codebank_funcs
                if func is not None
            ]
            codebank_str = "\n".join(codebank_str)
        ...
        if not self.task == "textcraft":
            prompt = llama_agent_prompt.format(
                codebank_str=codebank_instr, icl_string=icl_prompt, query=example.query
            )
        else:
            prompt = llama_agent_prompt.format(
                codebank_str=codebank_instr,
    ...
    def gpt_prompt_builder(self, example, icl_examples, craft_retrieve=False):
        ...
        # logo_agent_prompt
        if not self.task == "textcraft":
            prompt = gpt_agent_prompt.format(
                codebank_str=codebank_instr,
                # icl_string=icl_prompt,
                icl_string="None",
                query=example.query,
    ...
    def qwen_prompt_builder(self, example, icl_examples, craft_retrieve=False):
        """Build prompt for Qwen-style models"""
        icl_prompt = ""
        for ex in icl_examples:
            if self.use_thought:
                try:
        ...
        # Use Qwen-specific prompts from COMPLETION_PROMPTS dictionary
        qwen_agent_prompt = COMPLETION_PROMPTS[self.model_type][self.task]

        if not self.task == "textcraft":
            # For most tasks
            prompt = qwen_agent_prompt.format(
                codebank_str=codebank_instr, icl_string=icl_prompt, query=example.query
            )
        else:
            # For textcraft task
    ...
    def __call__(self, example, retry=False, craft_retrieve=False):
        ...
        if len(self.collections) == 1:
            icl_examples = self.retrieve_similar(
                "train", example, k=self.max_budget, exclude_current=True
            )
        else:
            n_tc_exs = int(self.budget_split * self.max_budget)
            testcase_examples = self.retrieve_similar(
                "test_cases", example, k=n_tc_exs, exclude_current=True
            )
            n_train_exs = self.max_budget - n_tc_exs
            train_examples = self.retrieve_similar(
                "train", example, k=n_train_exs, exclude_current=True
            )
        ...
        try:
            ...
            if not result and retry:
                feedback_prompt = FEEDBACK_PROMPTS[self.model_type][self.task].format(
                    codebank_str=self.codebank_instr,
                    crafting_commands=example.metadata,
                    query=example.query,
                    program=output,
                    exec_trace=err,
                ...
                retry_prompt = RETRIAL_PROMPTS[self.model_type][self.task].format(
                    codebank_str=self.codebank_instr,
                    crafting_commands=example.metadata,
                    query=example.query,
                    program=output,
    ...
    def do_multiple(self, examples, batch_size=5, rerun=False, exclude_current=True):
        if not rerun:
            ...
            for example in examples:
                if len(self.collections) == 1:
                    ...
                    n_train_exs = self.max_budget - n_tc_exs
                    train_examples = self.retrieve_similar(
                        "train", example, k=n_train_exs, exclude_current=exclude_current
                    )
                    icl_examples = testcase_examples + train_examples
            ...
            if hasattr(self.model, "run_multiple"):
                outputs = self.model.run_multiple(
                    prompts,
        ...
        print("Executing programs in do_multiples...")
        ...
        print("results, outputs in do multiple", results, outputs)
